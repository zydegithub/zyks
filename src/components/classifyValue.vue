...
export default {
  ...
  methods: {
      ...
      for (let index = 0; index < this.classNumber; index++) {
        ...
        for (let index2 = 0; index2 < this.fieldValues.length; index2++) {
          if (index == this.classNumber - 1) {
            if (
              this.fieldValues[index2] >= this.classifyArr[index * 2] &&
              this.fieldValues[index2] <= this.classifyArr[index * 2 + 1]
            ) {
              i++;
            }
          } else {
        ...
        var row = {
          value:
            this.classifyArr[index * 2].toFixed(2) +
            " - " +
            this.classifyArr[index * 2 + 1].toFixed(2),
          number: i
        };
      ...
      for (let i = 0; i < this.colorRandomArr.length; i++) {
        for (let index = 0; index < temp.length; index++) {
          if (this.colorRandomArr.length - 1 == i) {
            if (
              temp[index] >= this.classifyArr[i * 2] &&
              temp[index] <= this.classifyArr[i * 2 + 1]
            ) {
    ...
    finishEditClick() {
      ...
      var number1 = parseFloat(
        this.fieldList[this.currentEdit].value.substring(0, number - 1)
      );
      var number2 = parseFloat(
        this.fieldList[this.currentEdit].value.substring(number + 1)
      );
