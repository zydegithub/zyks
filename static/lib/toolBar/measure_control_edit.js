...
function MeasureControlEdit(options) {
  this.options = Object.assign({}, MeasureControlEdit.DEFAULTS, options);
 ...
 * @returns
 */
MeasureControlEdit.prototype.onAdd = function (map) {
  this._map = map;
 ...
 */
MeasureControlEdit.prototype.onRemove = function () {
 ...
 * @returns
 */
MeasureControlEdit.prototype.getDefaultPosition = function () {
  return 'top-left';
};
 ...
 * @returns {node}
 */
MeasureControlEdit.prototype._createNode = function (node, className, textContent, container, ariaLabel, fn) {
...
MeasureControlEdit.prototype._removeNode = function (node, fn) {
  node.remove();
 ...
 * @param {any} e
 */
MeasureControlEdit.prototype._onClick = function (e) {
  if (this.state == 'distanceOn') {
    this.drawLine(e);
    var point = [e.lngLat.lng, e.lngLat.lat]
    window.line.push(point)
  } else if (this.state == 'areaOn') {
    this.drawPolygon(e);
    var point = [e.lngLat.lng, e.lngLat.lat]
    window.polygon.push(point)
  }
};
// 测距模式下点击地图画线
MeasureControlEdit.prototype.drawLine = function (e) {
...
// 测面模式下点击地图画面
MeasureControlEdit.prototype.drawPolygon = function (e) {
  if (this.state === 'off') return;
 ...
 * @return {Number} 距离
 */
MeasureControlEdit.prototype._getLineDistance = function (lineString, unit) {
  return turf.lineDistance(lineString, unit).toLocaleString();
};
// 计算面积--单位：平方千米
MeasureControlEdit.prototype._getPolygonArea = function (polygon) {

  var area_mu = Number(turf.area(polygon) / 1000000).toFixed(2);
  return area_mu.toLocaleString();
};
/**
 * @description 获取所有的Point geojson格式
 * @return {Array} Point数组
 */
MeasureControlEdit.prototype._getPointsList = function () {
 ...
 */
MeasureControlEdit.prototype._removeCurPopups = function (name) {
 ...
 * @param {array} popup数组
 */
MeasureControlEdit.prototype._bindPointPopup = function (list) {
  var _this = this;
  ...
  if (this.state == 'distanceOn') {
    ...
    //     })
    //     .setLngLat(point.geometry.coordinates)
    //     .setHTML('面积：' + point.properties.area + '平方千米')
    //     .addTo(_this._map);
 ...
 * @param {any} e
 */
MeasureControlEdit.prototype._onDblClick = function (e) {
  if (this.state === 'off') return;
  ...
  if (this.clickFrom == 'distance') {
    // this.geojsonList[this._name].popups[length - 1].setHTML('<div>总距离：' + options.total + 'km' + '<button data-name=\'' + this._name + '\' id=\'dele-btn-' + this._name + '\'>X</button></div>');
    ...
    // this.geojsonList[this._name].popups[length - 1].setHTML('<div>总面积：' + options.area + '平方千米' + '<button class=close-btn data-name=\'' + this._name + '\' id=\'dele-btn-' + this._name + '\'>X</button></div>');
    // document.getElementById('dele-btn-' + this._name).style.marginLeft = '5px';
    // document.getElementById('dele-btn-' + this._name).addEventListener('click', this._removeMeasure);
  }
  this.curGeojson = {
 ...
 */
MeasureControlEdit.prototype._removeMeasure = function (e) {
 ...
 * @param {any} e
 */
MeasureControlEdit.prototype._onMeasureStart = function (e) {
  window.line = []
  // if (this.state == 'off') {
 ...
 * @param {any} e
 */
MeasureControlEdit.prototype._onMeasureAreaStart = function (e) {
  window.polygon = []
  // if (this.state == 'off') {
 ...
 * @param {event} 鼠标点击事件
 */
MeasureControlEdit.prototype._onAddMouseTips = function (e) {
 ...
 */
MeasureControlEdit.prototype._setTipsPosition = function (e, tips) {
  if (!e || !tips) return;
...
/**
 * @description 创建MeasureControlEdit的mapSource、mapLayer
 */
MeasureControlEdit.prototype._onAddSourceLayer = function () {
  // 样式
 ...
 * @description 创建MeasureControlEdit的mapSource、mapLayer
 */
MeasureControlEdit.prototype._onAddAreaSourceLayer = function () {
  // 样式
  this.clickFrom = 'area';
  var circleRadius = this.options.circleRadius;
  var circleColor = this.options.circleColor;
 ...
 * @param {any} e
 */
MeasureControlEdit.prototype._onMeasureEnd = function (e) {
  console.log('结束测距模式');
  this._toggleState('off');
};
 ...
 * @param {any} e
 */
MeasureControlEdit.prototype._onMousemove = function (e) {
  if (this.state === 'off') return;
 ...
 */
MeasureControlEdit.prototype._toggleState = function (state) {
  this.state = state;
  // 禁用map双击放大效果
  if (state === 'on') {
 ...
 * @param {string} 鼠标手势
 */
MeasureControlEdit.prototype._toggleCursor = function (cursor) {
 ...
 * @param {string} 事件名称
 */
MeasureControlEdit.prototype._toggleEvents = function (toggle, event) {
  var _this = this;
